...
private _show_additional_sides = [["blufor", false], ["opfor", false], ["independent", false], ["civilian", false]];
private _ui_color_list_rgba = [0, 0.42, 1, 1];   
private _ui_color_confirm_rgba = [0, 0.42, 1, 0.8];   
private _ui_color_cancel_rgba = [0, 0.42, 1, 1];   
...
ARTEK_fnc_startOperatorFeed = {     
    ...
    [_monitor] spawn {     
        ...
        private _offset = missionNamespace getVariable ["ARTEK_helmet_cam_offset", [0.19, 0.05, 0.05]];  
        private _pitch = missionNamespace getVariable ["ARTEK_helmet_cam_pitch", -15];  
        private _yaw = missionNamespace getVariable ["ARTEK_helmet_cam_yaw", -10];  
        private _roll = missionNamespace getVariable ["ARTEK_helmet_cam_roll", 0];  
        private _fov = missionNamespace getVariable ["ARTEK_helmet_cam_fov", 0.85];  
        ...
        while {_monitor getVariable ["operatorFeedActive", false] && !isNull _cam} do {     
            ...
            private _dirVec = _modelMatrix select 0; 
            private _upVec = _modelMatrix select 1; 
            private _rightVec = _dirVec vectorCrossProduct _upVec; 
             
 
            private _offsetVec =  
                (_rightVec vectorMultiply (_offset select 0)) vectorAdd 
                (_dirVec vectorMultiply (_offset select 1)) vectorAdd 
                (_upVec vectorMultiply (_offset select 2)); 
            ...
            private _camUpVec = [  
                -sin(_finalYaw) * sin(_rollRad),  
                -cos(_finalYaw) * sin(_rollRad),  
                cos(_rollRad)  
            ];  
